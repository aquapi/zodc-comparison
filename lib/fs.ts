...
export const scanFiles = (pat: string, cwd: string) =>
  Iterator.from(
...
export const tryWriteAsync = (path: string, content: string) =>
  writeFile(path, content, (err) => {
    if (err !== null) console.error('Failed to write file:', fmt.relativePath(path));
    else console.log('File written successfully:', fmt.relativePath(path));
  });
